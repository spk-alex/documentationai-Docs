...
2. Check that your subscription includes the correct event types (or no `events` filter at all — see Known Issues below).
...
### Event-name mismatch between subscription schema and emitted events

The admin webhook-creation API currently validates `events[]` against a legacy kebab-case enum (`lock.opened`, `device.added`, `key.created`, …). The B2C API actually emits PascalCase `detail-type`s (`KeyAdded`, `LockCreated`, `ResidentAdded`, …). **Subscribing to one of the legacy enum values will not match any real event.**

Workaround until the schema is fixed:

- Ask your account manager to subscribe you with an empty `detail_types` filter — EventBridge will then deliver every event tagged with your `partnerId`. The underlying Lambda accepts an empty array; only the admin-API schema currently rejects it.
- Once delivered, filter on `detail-type` in your handler using the PascalCase names listed above.

The `Shadow Update` event is the one entry that already matches across both the schema and the emitted name.
