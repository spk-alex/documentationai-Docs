...
Register a webhook URL to receive events for this partner.
...
| Event | Description |
|-------|-------------|
| `access-control.*` | All access control events (wildcard) |
| `device.added` | Device added to a lock |
| `device.removed` | Device removed from a lock |
| `key.created` | New key assigned |
| `key.revoked` | Key revoked |
| `lock.opened` | Lock opened |
| `lock.access-denied` | Lock access denied |
| `subscription.created` | Webhook subscription created |
| `subscription.updated` | Webhook subscription updated |
| `subscription.deleted` | Webhook subscription deleted |
  ...
  "webhooks": [
    {
      ...
      "url": "https://acme-corp.com/webhooks/smartphonekey",
      "events": ["device.added", "device.removed", "key.created", "key.revoked", "lock.opened", "lock.access-denied"],
      "status": "pending",
...
curl -X PUT "https://admin-api.spk-dev.workers.dev/tenants/acme-corp/webhooks/WEBHOOK_ID" \
  ...
  -d '{
    ...
    "events": ["device.added", "lock.opened"]
  }'
```
