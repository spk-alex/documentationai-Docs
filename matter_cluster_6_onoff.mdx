...
Some Matter libraries may require PascalCase field names:
...
def can_send_cluster6_command(shadow: dict, endpoint_id: int, command_name: str) -> bool:
    accepted_key = f"{endpoint_id}/6/65529"
    accepted = shadow.get(accepted_key, [])
    command_id = COMMAND_IDS.get(command_name)

    if command_id is None:
        return False

    return command_id in accepted
```

## State Interpretation Example

```python
def get_cluster6_state(shadow: dict, endpoint_id: int) -> str:
    state_key = f"{endpoint_id}/6/0"
    value = shadow.get(state_key)
