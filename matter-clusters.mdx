---
title: Matter Device Attributes & Commands
...
| --- | --- | --- |
| Basic Information (40) | VendorName | `0/40/1` |
| Basic Information (40) | ProductName | `0/40/3` |
| Basic Information (40) | HardwareVersionString | `0/40/8` |
| Basic Information (40) | SoftwareVersionString | `0/40/10` |
| Basic Information (40) | ManufacturingDate/DateCode | `0/40/11` |
| Basic Information (40) | SerialNumber | `0/40/15` |
| Basic Information (40) | VendorID | `0/40/2` |
| Basic Information (40) | ProductURL | `0/40/14` |
| General Diagnostics (51) | UpTime | `0/51/2` |
| General Diagnostics (51) | TotalOperationalHours | `0/51/3` |
| General Diagnostics (51) | BootReason | `0/51/4` |
| Thread Network Diagnostics (53) | NeighborTable | `0/53/7` |
| Thread Network Diagnostics (53) | RoutingRole | `0/53/1` |
| Thread Network Diagnostics (53) | NetworkName | `0/53/2` |
| Wi-Fi Network Diagnostics (54) | RSSI | `0/54/4` |
| ICD Management (70) | IdleModeDuration | `0/70/0` |
| ICD Management (70) | ActiveModeDuration | `0/70/1` |
| ICD Management (70) | ActiveModeThreshold | `0/70/2` |
| ICD Management (70) | UserActiveModeTriggerHint | `0/70/6` |
| ICD Management (70) | UserActiveModeTriggerInstruction | `0/70/7` |
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Door Lock (257) | LockState | `1/257/0` |
| Door Lock (257) | NumberOfTotalUsersSupported | `1/257/17` |
| Door Lock (257) | NumberOfPINUsersSupported | `1/257/18` |
| Door Lock (257) | NumberOfCredentialsSupportedPerUser | `1/257/23` |
| Door Lock (257) | AutoRelockTime | `1/257/27` |
| Door Lock (257) | SoundVolume | `1/257/28` |
| Door Lock (257) | LEDSettings | `1/257/41` |
| Door Lock (257) | AutoRelockBehavior | `1/257/43` |
| Door Lock (257) | WrongCodeEntryLimit | `1/257/48` |
| Door Lock (257) | UserCodeTemporaryDisableTime | `1/257/49` |
| Door Lock (257) | RequirePINforRemoteOperation | `1/257/51` |
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Power Source (47) | Status | `1/47/0` |
| Power Source (47) | BatVoltage | `1/47/11` |
| Power Source (47) | BatPercentRemaining | `1/47/12` |
| Power Source (47) | BatChargeLevel | `1/47/14` |
| Power Source (47) | BatReplacementNeeded | `1/47/15` |
| Power Source (47) | BatReplacementDescription | `1/47/19` |
| Power Source (47) | BatReplacementNeededLevel | `1/47/24` |
| Power Source (47) | BatQuantity | `1/47/25` |
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Relative Humidity Measurement (1029) | MeasuredValue | `1/1029/0` |
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Occupancy Sensing (1030) | Occupancy | `1/1030/0` |
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Boolean State (69) | StateValue | `1/69/0` |

**StateValue** — Binary state value. For this door sensor it maps to open/closed contact state. Example: `true`. Boolean value. For a contact sensor endpoint, map with endpoint context: commonly `true = closed/contact inactive`, `false = open/contact active`, but verify with the specific device behavior.
...
Binary state value. On this interpretation it maps to water leak state because the endpoint reports Water Leak Detector. Example: `false`. Boolean value. For a water leak detector endpoint, `true` normally means leak detected/alarm active; `false` means no leak detected.
...
| Cluster | Attribute | Path |
| --- | --- | --- |
| Temperature Measurement (1026) | MeasuredValue | `2/1026/0` |
...
These rows show whether the endpoint currently detects smoke locally or receives a smoke alarm state from another interconnected smoke detector.

| Cluster | Attribute | Path |
|---|---|---|
| Smoke CO Alarm (92) | SmokeState | `1/92/0` |
| Smoke CO Alarm (92) | InterconnectSmokeAlarm | `1/92/7` |

**SmokeState** — Local smoke state of this exact detector.

```text
0 = Normal / no local smoke detected
1 = Warning / local smoke warning
2 = Critical / local smoke alarm
```

**InterconnectSmokeAlarm** — Smoke alarm state received from another detector in the same interconnected alarm group.

```text
0 = Normal / no interconnected smoke alarm
1 = Warning / another detector reports smoke warning
2 = Critical / another detector reports critical smoke alarm
```

#### Difference

`SmokeState` means this detector itself detects smoke.

`InterconnectSmokeAlarm` means another linked detector reports smoke, and this detector exposes that alarm state.
...
This means this detector does not detect smoke locally, but another interconnected detector reports a critical smoke alarm.

#### Interpretation

| `1/92/0` SmokeState | `1/92/7` InterconnectSmokeAlarm | Local smoke | Interconnected smoke | Final status |
|---|---|---|---|---|
| `0` | `0` | No | No | Normal / no smoke detected |
| `1` | `0` | Warning | No | Local smoke warning |
| `2` | `0` | Critical | No | Local critical smoke alarm |
| `0` | `1` | No | Warning | Another detector reports smoke warning |
| `0` | `2` | No | Critical | Another detector reports critical smoke alarm |
| `1` | `1` | Warning | Warning | Local and interconnected smoke warning |
| `1` | `2` | Warning | Critical | Interconnected critical smoke alarm active |
| `2` | `1` | Critical | Warning | Local critical smoke alarm active |
| `2` | `2` | Critical | Critical | Critical smoke alarm active |
...
| Path | Cluster ID | Cluster Name | Accepted Command IDs | Decoded Commands | What this means |
| --- | ---: | --- | --- | --- | --- |
| `0/48/65529` | 48 / `0x0030` | General Commissioning | `[0, 2, 4]` | `0 / 0x00 = ArmFailSafe`<br />`2 / 0x02 = SetRegulatoryConfig`<br />`4 / 0x04 = CommissioningComplete` | Device supports the standard commissioning safety flow. These commands are used while adding the device to a Matter fabric. |
| `0/49/65529` | 49 / `0x0031` | Network Commissioning | `[0, 3, 4, 6, 8]` | `0 / 0x00 = ScanNetworks`<br />`3 / 0x03 = AddOrUpdateThreadNetwork`<br />`4 / 0x04 = RemoveNetwork`<br />`6 / 0x06 = ConnectNetwork`<br />`8 / 0x08 = ReorderNetwork` | Device supports network commissioning operations. Because this list has `AddOrUpdateThreadNetwork` and not Wi-Fi network commands, this looks like Thread commissioning capability. |
| `0/50/65529` | 50 / `0x0032` | Diagnostic Logs | `[0]` | `0 / 0x00 = RetrieveLogsRequest` | Controller can request diagnostic logs from the device if the device has logs available. |
| `0/51/65529` | 51 / `0x0033` | General Diagnostics | `[0, 1]` | `0 / 0x00 = TestEventTrigger`<br />`1 / 0x01 = TimeSnapshot` | Device supports diagnostic commands. These are mainly useful for testing, certification, troubleshooting, and time snapshot diagnostics. |
| `0/53/65529` | 53 / `0x0035` | Thread Network Diagnostics | `[0]` | `0 / 0x00 = ResetCounts` | Controller can reset Thread diagnostic counters. This is diagnostics-only and does not control the main device function. |
| `0/60/65529` | 60 / `0x003C` | Administrator Commissioning | `[0, 1, 2]` | `0 / 0x00 = OpenCommissioningWindow`<br />`1 / 0x01 = OpenBasicCommissioningWindow`<br />`2 / 0x02 = RevokeCommissioning` | Device can open or revoke a commissioning window. This is used for adding another controller/admin or putting the device back into commissioning mode. |
| `0/62/65529` | 62 / `0x003E` | Operational Credentials | `[0, 2, 4, 6, 7, 9, 10, 11]` | `0 / 0x00 = AttestationRequest`<br />`2 / 0x02 = CertificateChainRequest`<br />`4 / 0x04 = CSRRequest`<br />`6 / 0x06 = AddNOC`<br />`7 / 0x07 = UpdateNOC`<br />`9 / 0x09 = UpdateFabricLabel`<br />`10 / 0x0A = RemoveFabric`<br />`11 / 0x0B = AddTrustedRootCertificate` | Device supports Matter fabric/security certificate operations. These commands are used during commissioning, fabric setup, multi-admin, and operational credential management. |
| `0/63/65529` | 63 / `0x003F` | Group Key Management | `[0, 1, 3, 4]` | `0 / 0x00 = KeySetWrite`<br />`1 / 0x01 = KeySetRead`<br />`3 / 0x03 = KeySetRemove`<br />`4 / 0x04 = KeySetReadAllIndices` | Device supports group key management for secure Matter group communication. This is not a user-facing device action. |
...
| Decimal value | Matter Device Type             | Bridge Internal Type               |
| ------------: | ------------------------------ | ---------------------------------- |
|        `10` | Door Lock                      | `lock`                           |
